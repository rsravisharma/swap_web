...
@section('page-title', 'Edit PDF Book')
...
<div class="p-6">
    ...
    <div class="bg-white rounded-xl shadow">
        <div class="p-6 border-b">
            <h2 class="text-2xl font-bold text-gray-900">Edit PDF Book</h2>
            <p class="text-sm text-gray-600 mt-1">Update the book details</p>
        </div>

        <form method="POST" action="{{ route('admin.pdf-manager.update', $book->id) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select name="user_id"
                    ...
                    <option value="{{ $user->id }}" {{ (old('user_id', $book->seller_id) == $user->id) ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="language"
                        ...
                        <option value="hi" {{ old('language', $book->language) === 'hi' ? 'selected' : '' }}>Hindi</option>
                ...
                <div>
                    ...
                    <input type="number"
                        name="file_size"
                        value="{{ old('file_size', $book->file_size) }}"
                        min="1"
                        placeholder="5242880"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text"
                        ...
                        value="{{ old('google_drive_file_id', $book->google_drive_file_id) }}"
                        required
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="url"
                        ...
                        value="{{ old('google_drive_shareable_link', $book->google_drive_shareable_link) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <img src="{{ $book->cover_image_url }}" alt="{{ $book->title }}" class="w-32 h-40 object-cover rounded-lg shadow">
                </div>
                @endif
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $book->cover_image_url ? 'Change Cover Image' : 'Cover Image' }}
                    </label>
                    <input type="file"
                        name="cover_image"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description"
                        id="description"
                        class="hidden @error('description') border-red-500 @enderror">{{ old('description', $book->description) }}</textarea>
                ...
                <!-- Availability -->
                <div class="md:col-span-2 mt-10 pt-10">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm font-medium text-gray-700">Book is available for purchase</span>
                    </label>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // ====== QUILL (already in your edit file) ======
        ...
        // ====== CATEGORY (copied from create, with prefill) ======
        ...
        function loadChildren(parentId, targetSelect, selectedId = null, callback = null) {
            if (!parentId) return;
            ...
            fetch(`/admin/categories/${parentId}/children`)
                ...
                .then(children => {
                    ...
                    children.forEach(child => {
                        const selectedAttr = selectedId && selectedId == child.id ? 'selected' : '';
                        targetSelect.innerHTML += `<option value="${child.id}" ${selectedAttr}>${child.name}</option>`;
                    });
        ...
        // Change handlers (same as create)
        ...
        // ====== PREFILL CURRENT CATEGORY ======
        const currentCategoryId = finalCategoryInput.value;

        if (currentCategoryId) {
            // You probably know only the leaf id here.
            // Simple, practical approach:
            // 1) Set finalCategoryInput as selected preview text from backend (optional),
            // 2) If you know main/sub/child hierarchy in controller, you can pass:
            //    $currentMainId, $currentSubId, $currentChildId and use them here.

            // Example if you pass from controller:
            // $currentMainId, $currentSubId, $currentChildId
            @if(isset($currentMainId))
            mainSelect.value = '{{ $currentMainId }}';
            previewText.textContent = `📂 ${mainSelect.options[mainSelect.selectedIndex].text}`;
            previewDiv.classList.remove('hidden');

            // load subcategories and select current sub
            loadChildren('{{ $currentMainId }}', subSelect, '{{ $currentSubId ?? '
                ' }}',
                function() {
                    // load child categories and select current child
                    @if(isset($currentSubId))
                    childSelect.disabled = false;
                    loadChildren('{{ $currentSubId }}', childSelect, '{{ $currentChildId ?? '
                        ' }}');
                    @endif
                });
            @endif
