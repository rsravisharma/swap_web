    ...
    <div class="max-w-md w-full space-y-8">
        <div>
            ...
            <p class="mt-2 text-center text-sm text-gray-600">
                Access your PDF library and purchases
            </p>
        </div>

        <!-- Email/Password Form -->
        <form class="mt-8 space-y-6 bg-white p-8 rounded-2xl shadow-xl" id="loginForm">
            @csrf
            <div class="space-y-4">
                <div>
            ...
            <div class="text-center">
                <p class="text-sm text-gray-600">
                    ...
                    <a href="{{ route('user.register') }}" class="font-medium text-primary-600 hover:text-primary-700">Register now</a>
                </p>
            </div>
...
<script>
    document.getElementById('loginForm').addEventListener('submit', async function(e) {
        ...
        submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm mr-2" role="status"></span>Signing in...';
        submitBtn.disabled = true;

        try {
            const formData = new FormData(this);

            const response = await fetch('{{ route("user.login.submit") }}', {
                method: 'POST',
                body: formData,
                headers: {
